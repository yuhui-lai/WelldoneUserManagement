...
export default defineComponent({
    ...
    data() {
        return {
            showDialog: true,
            getBasicInfoApi: '/api/Role/BasicInfo/',
            editRoleBasicInfoApi: '/api/Role/EditBasicInfo',
            reqModel: {
                OperatorId: '',
                RoleId: '',
                ProductCode: '',
    ...
    methods: {
        async Edit() {
            ...
            fetch(this.editRoleBasicInfoApi, {
                method: "PUT",
                headers: headers,
        ...
        GetBasicInfo() {
            ...
            fetch(`${this.getBasicInfoApi}${this.editId}`, {
                method: "GET",
                headers: headers
            })
                .then(r => {
                ...
                .then(json => {
                    this.reqModel.ProductCode = json.data.productCode;
                    this.reqModel.RoleName = json.data.roleName;
                    this.reqModel.RoleId = this.editId;
                    const productAllOption = [{ title: '', value: '' }];
                    this.productOptions = productAllOption.concat(json.data.products);
                    this.loading = false;
                    return;
                }).catch(error => {
                    this.FetchErrorHandle(error);
